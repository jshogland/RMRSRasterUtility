    ...
    public class batchCalculations
    {
        ...
        public void runBatch()
        {
            ...
            finally
            {
                ...
                rp.stepPGBar(100);
                rp.addMessage("Finished Batch Process in " + tsStr);
                rp.enableClose();
            }
        }
        ...
        private ITable selectSamples(string[] paramArr)
        {
            ...
            if (Int32.TryParse(mdl, out ns))
            {
                ...
                if (sFld == "")
                {
                    ftrUtil.selectRandomFeaturesToSample(inTbl, System.Convert.ToInt32(mdl));
                }
                else
                { 
        ...
        private string buildModels(string[] paramArr)
        {
            ...
            switch (mType)
            {
                ...
                case Statistics.dataPrepBase.modelTypes.CompareClassifications:
                    ...
                    break;
                case Statistics.dataPrepBase.modelTypes.Normalize:
                    createNormalizeFunction(paramArr);
                    break;
                default:
                    break;
            }
        ...
        private void createNormalizeFunction(string[] paramArr)
        {
            IFunctionRasterDataset rs1 = getRaster(paramArr[1]);
            IFunctionRasterDataset rs2 = getRaster(paramArr[2]);
            int pc = System.Convert.ToInt32(paramArr[3]);
            normalization nm = new normalization(rs1, rs2, pc, rsUtil);
            IFunctionRasterDataset rs3 = nm.OutRaster;
            nm.writeModel(paramArr[4]);
        }
