...
namespace esriUtil.Forms.Stats
{
    public partial class frmSummarizeRelatedTable : Form
    {
        ...
        private void btnExecute_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                IWorkspaceEdit wksEdit = (IWorkspaceEdit)((IDataset)ptbl).Workspace;
                if (wksEdit.IsBeingEdited())
                {
                    rp.addMessage("Edit session is open. You must close the current edit session before using this tool!");
                }
                else
                {
