    ...
    public class TransRouting
    {
        ...
        private void convertFeatureClass()
        {
            if (RoadRaster == null)
            {
                ...
                IRasterDataset geoDset = convOp.ToRasterDataset((IGeoDataset)ftrDesc, "TIFF", OutWorkspace, outRdName);
        ...
        private void buildPathRasters()
        {
            ...
            IFunctionRasterDataset t1 = rsUtil.calcArithmaticFunction(t0, 2, esriRasterArithmeticOperation.esriRasterMultiply); //round trip
        ...
        private void buildPathAllocation()
        {
            ...
            rasterAnalysisEnvironment.OutWorkspace = OutWorkspace;
            object ext = getExtent();//((IGeoDataset)RoadFeatureClass).Extent;
            object snap = ((IGeoDataset)Dem);
            ...
            System.Runtime.InteropServices.Marshal.ReleaseComObject(rpdOut);
            FunctionAccumulatedPathDistance = rsUtil.createIdentityRaster(OutWorkspace.PathName + "\\" + datasetName.Name);
            ...
            if (BarrierRaster != null || BarriersFeatureClass != null)
            {
            ...
            Console.WriteLine("made it to off road costs");
            IGeoDataset rpdOut2 = dOp.PathDistance((IGeoDataset)rdCs, costRS, Dem, noVl, noVl, noVl, noVl, noVl, noVl);
            System.Runtime.InteropServices.Marshal.ReleaseComObject(rpdOut2);

            FunctionAccumulatedFromPathDistance = rsUtil.createIdentityRaster(OutWorkspace.PathName + "\\" + datasetName.Name);
            string frRoadAllo = rsUtil.getSafeOutputName(OutWorkspace, "onRoadAllo");
            datasetName.Name = frRoadAllo;
            rasOpBase.AddOutputDatasetName(0, datasetName);
            IGeoDataset rpdOut3 = dOp.PathAllocation((IGeoDataset)rdCs, costRS, Dem, noVl, noVl, noVl, noVl, noVl, noVl);
            System.Runtime.InteropServices.Marshal.ReleaseComObject(rpdOut3);
            FunctionAccumulatedPathAllocation = rsUtil.createIdentityRaster(OutWorkspace.PathName + "\\" + datasetName.Name);
        }

        private object getExtent()
        {
            object outExt = new EnvelopeClass();
            if (RoadFeatureClass != null)
            {
                outExt = ((IGeoDataset)RoadFeatureClass).Extent;
            }
            if (RoadFeatureClass==null && RoadRaster != null)
            {
                outExt = RoadRaster.RasterInfo.Extent;
            }
            return outExt;
        }
